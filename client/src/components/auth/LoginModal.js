...
const LoginModal = ({ isOpen, onClose, onLogin, onShowAlert }) => {
    ...
    const sendOtpEmail = async (targetEmail, targetName = "User") => {
        ...
        try {
            ...
            setError("Gagal mengirim kode OTP. Cek koneksi internet.");
            return false;
        }
    };
    ...
    const handleUserLogin = async (e) => {
        ...
        } catch (err) { setError('Server tidak merespon.'); }
        setIsLoading(false);
    };
    ...
    const handleRegisterRequest = async (e) => {
        ...
        // A. Cek dulu apakah email sudah terdaftar
        ...
        // B. Jika belum, Kirim OTP
        const isSent = await sendOtpEmail(email, name);
        setIsLoading(false);
        
        if (isSent) {
            onShowAlert({ title: 'OTP Terkirim', message: `Kode verifikasi dikirim ke ${email}`, type: 'alert' });
    ...
    const handleVerifyRegister = async (e) => {
        ...
        if (inputOtp === otpCode) {
            ...
            try {
                ...
                if (data.status === 'success') {
                    onShowAlert({ title: 'Sukses', message: 'Akun dibuat! Silakan Login.', type: 'alert' });
                    resetAll();
                } else {
                    setError(data.message || 'Gagal mendaftar.');
                }
            } catch (err) { setError('Terjadi kesalahan server.'); }
            setIsLoading(false);
    ...
    const handleForgotRequest = async (e) => {
        ...
        // A. Cek apakah email ADA di database
        const isExist = await checkEmailExists(email);
        if (!isExist) {
        ...
        // B. Kirim OTP
        const isSent = await sendOtpEmail(email, "User");
        setIsLoading(false);

        if (isSent) {
            onShowAlert({ title: 'OTP Terkirim', message: `Kode reset dikirim ke ${email}`, type: 'alert' });
            setView('otp_forgot');
        }
    };

    // 5. VERIFIKASI FORGOT OTP
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            ...
            if (data.status === 'success') {
                onShowAlert({ title: 'Sukses', message: 'Password berhasil diubah! Silakan login.', type: 'alert' });
                resetAll();
            } else {
    ...
    const handleGoogleAuth = async () => {
        ...
        try {
            ...
            if (data.status === 'success') {
                onLogin(data.user); 
                onClose();
            } else { setError("Gagal sinkronisasi data Google ke Server."); }

        } catch (error) { setError("Gagal login dengan Google."); }
        setIsLoading(false);
    };
   ...
   return (
       <div className="modal-overlay" onClick={onClose}>
           <div className="auth-card" onClick={(e) => e.stopPropagation()}>
               ...
               {/* FORM LOGIN */}
               {view === 'login' && (
                   <form onSubmit={handleUserLogin} style={{display:'flex', flexDirection:'column', gap:'15px'}}>
               ...
               {(view === 'otp_register' || view === 'otp_forgot') && (
                   <form onSubmit={view === 'otp_register' ? handleVerifyRegister : handleVerifyForgot} style={{display:'flex', flexDirection:'column', gap:'15px'}}>
                       ...
                       <button type="submit" className="btn-primary" disabled={isLoading}>VERIFIKASI</button>
               ...
               {view === 'reset_password' && (
                   <form onSubmit={handleResetPassword} style={{display:'flex', flexDirection:'column', gap:'15px'}}>
                       ...
                       <button type="submit" className="btn-primary" disabled={isLoading}>{isLoading ? 'Menyimpan...' : 'SIMPAN PASSWORD'}</button>
                   </form>
               )}
